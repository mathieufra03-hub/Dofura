...
function Navbar({ onHome }) {
  return (
    <nav style={{
      background: C.bg2, borderBottom: `0.5px solid ${C.bdr2}`,
      padding: "0 2rem", display: "flex", alignItems: "center",
      height: 48, position: "sticky", top: 0, zIndex: 100,
    }}>
      <span onClick={onHome} style={{
        fontFamily: "'Cinzel', serif", fontWeight: 900, fontSize: 17,
        background: "linear-gradient(90deg,#f0c040,#c478ff)",
        WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent",
        letterSpacing: "0.08em", marginRight: 28, cursor: "pointer", flexShrink: 0,
      }}>\u00b7 DOFURA \u00b7</span>
      ...
      <div style={{ marginLeft: "auto", display: "flex", gap: 8, alignItems: "center" }}>
        <div style={{ background: C.bg3, border: `0.5px solid ${C.cyanb}`, borderRadius: 6, padding: "5px 11px", fontSize: 12, color: C.txt3, display: "flex", alignItems: "center", gap: 5 }}>
          \ud83d\udd0d Ctrl K
        </div>
...
function StatsBar() {
  const items = [
    ...
    { val: "4 210", label: "qu\\u00eates" },
    { val: "18 900", label: "articles" },
    { val: "18", label: "classes" },
    { val: "1 430", label: "succ\\u00e8s" },
    { val: "18", label: "m\\u00e9tiers" },
  ]
  return (
...
function EncycloGrid({ onMonsters }) {
  const items = [
    ...
    { icon:"\ud83d\udcdc", label:"Qu\\u00eates",    count:"4 210",  action: null },
    { icon:"\ud83d\udee1\ufe0f", label:"Objets",    count:"18 900", action: null },
    { icon:"\ud83d\udd28", label:"M\\u00e9tiers",   count:"18",     action: null },
    ...
    <div style={{ padding:"18px 2rem 0" }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
        <span style={{ fontSize:13, fontWeight:500, color: C.txt, display:"flex", alignItems:"center", gap:7 }}>
          <span style={{ color: C.cyan }}>\ud83d\udcda</span> Encyclop\\u00e9die
        </span>
...
function MonstrePage({ id, onBack }) {
  ...
  if (!data) return (
    <div style={{ padding:"3rem 2rem", textAlign:"center", color: C.txt2, fontSize:14 }}>Chargement...</div>
  )
          ...
          <div style={{ display:"flex", gap:4, flexWrap:"wrap" }}>
            {data.grades?.map((gr, i) => (
              <button key={i} onClick={() => setGradeIdx(i)} style={{
                ...
                color: i === gradeIdx ? C.cyan : C.txt2, cursor:"pointer",
...
print("OK — App.jsx réécrit proprement")