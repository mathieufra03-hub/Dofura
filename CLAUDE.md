...
10. **Chantier #7 — Objets/Équipements (Phase 1 roadmap) — SCRAPING FAIT (2026-07-09 nuit), INTÉGRATION EN ATTENTE.** Session nocturne autonome (mode sans surveillance validé par Popo). 4 endpoints DofusDB identifiés et testés : `/items` (21 738), `/recipes` (4 858, `ingredientIds`+`quantities`+`jobId`), `/item-sets` (928 panoplies, bonus progressifs par nombre de pièces), `/item-types` (239 catégories). Pagination en liste ($limit/$skip, plafond 50/page) au lieu de requêtes individuelles — ~557 pages au total. **Bug trouvé et corrigé pendant la nuit :** la fonction de log elle-même plantait tout le script sur une erreur `Permission denied` transitoire (fichier verrouillé), un comble vu que "jamais un crash" était l'exigence n°1 — corrigé en rendant `log()`/`sauver_donnees()`/`sauver_progress()` elles-mêmes increvables (retry, puis abandon silencieux de cette seule ligne sans jamais interrompre le scraping). Reprise sur incident testée en conditions réelles (crash à skip=3000 sur les recettes, redémarrage reparti pile de là, 0 donnée perdue). **Résultat final : les 4 collections à 100%, 0 erreur, 0 doublon, comptes exacts vérifiés.**
    **⚠️ Point bloquant découvert, non résolu cette nuit (hors mandat : pas de décision d'architecture seule) :** `dofura_recipes.json` (244 Mo) et `dofura_item_sets.json` (55,7 Mo) intègrent l'objet complet de chaque item référencé (`result`/`ingredients` pour les recettes, `items` pour les panoplies) au lieu d'un simple ID — la même ressource très utilisée (ex. un composant de craft utilisé dans des centaines de recettes) est donc dupliquée intégralement à chaque fois. `dofura_items.json` (211 Mo) est gros mais légitimement (chaque item porte ses propres tableaux, parfois longs — ex. `dropMonsterIds` avec 5 000+ entrées pour une ressource commune). **Les 3 fichiers dépassent largement la limite GitHub de 100 Mo/fichier — impossible de les pousser en l'état.** Non commités, non supprimés, laissés sur disque pour la session de demain. Options à trancher ensemble : (a) ne garder que les IDs dans recipes/item-sets (les vraies données sont déjà dans items.json — dédoublonnage complet) ; (b) filtrer items.json à un sous-ensemble de champs utiles (ex. retirer `dropMonsterIds`/`recipeIds` complets si non utilisés à l'affichage) ; (c) autre approche. Seuls commités/poussés cette nuit : le scraper corrigé (`scripts/scraper_items.py`, gardé — réutilisable comme les autres `scraper_*.py`) et `dofura_item_types.json` (240 Ko, propre, aucune ambiguïté). Log complet : `scripts/scraper_items_log.txt` (non commité, à lire en local).
