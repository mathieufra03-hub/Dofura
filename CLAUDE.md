...
7. **Chantier #5 — Lisibilité bonus/malus des effets (ouvert 2026-07-08).** Signalement initial : Bêlement affichait "2 PM" sans signe, soupçonné d'être un malus (devrait être "-2 PM"). Enquête (lecture seule) sur `api.dofusdb.fr/effects/{id}` : chaque effet a `characteristicOperator` ("+"/"-"), `boost`, `bonusType` et un `oppositeId` vers son inverse — des champs qu'on ne stocke PAS dans `dofura_effects.json` (seuls `description`/`is_in_percent` sont gardés). Constat clé : Ankama écrit le signe "-" **en dur dans le texte français** pour les malus (ex. effet 169 "-#1{{~1~2 à -}}#2 PM"), mais jamais de "+" pour les bonus (ex. effet 128 "#1{{~1~2 à }}#2 PM") — un nombre sans signe est donc censé se lire comme un bonus par convention Ankama. **Bêlement utilise l'effet 128 (bonus, confirmé en jeu par Popo le 2026-07-08 : +2/+3 PM, pas un malus)** — le signalement initial était une fausse alerte, pas un bug. Le vrai chantier qui en ressort : les bonus PM/PA/Esquive (effets 128, 161, etc., ~557 sorts rien que pour l'effet 128) s'affichent sans "+" explicite alors que les malus équivalents affichent bien leur "-" — ambiguïté de lisibilité, pas un signe faux. Plan proposé, pas encore validé par Popo — voir conversation du 2026-07-08.
